...
server.log("Get Temperature:" + http.agenturl() + "?sensor=T&password=your_password") ;
server.log("Get RHumidity:" + http.agenturl() + "?sensor=RH&password=your_password") ;
server.log("Get RHumidity:" + http.agenturl() + "?sensor=P&password=your_password") ;
server.log("Get Illuminace:" + http.agenturl() + "?sensor=I&password=your_password") ;
server.log("Get ALL sensors:" + http.agenturl() + "?sensor=ALL&password=your_password") ;
// password check
...
function requestHandler(request, response) {
  try {
      if (BlobToHexString(http.hash.sha1(request.query.password))=="f6d42f9189be4cc8f9f6d75498eab0005f4c7a7b")
      {
    ...
    if ("sensor" in request.query) {
     ...
     } else {response.send(500, " Error password ") ;}
  } catch (ex) {
